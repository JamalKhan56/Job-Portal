...
// Fetch job applications from apply_job_post where id_company is the current company
$sqlApplications = "SELECT apply_job_post.id_user, apply_job_post.id_jobpost, job_post.jobtitle, users.firstname, users.lastname
                   FROM apply_job_post
                   ...
                   WHERE apply_job_post.id_company = '$id_company'";
...
if ($resultApplications->num_rows > 0) {
    while ($rowApplication = $resultApplications->fetch_assoc()) {
        echo '<p>' . $rowApplication['jobtitle'] . ' - ' . $rowApplication['firstname'] . ' ' . $rowApplication['lastname'] . '</p>';
    }
} else {
