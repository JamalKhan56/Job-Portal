...
<div class="content">
    ...
    <div class="blue-text">
        ...
        <div class="container">
            <img src="../images/logoimg.jpg"> <p> Jobs Posted <br> <span><?php echo getNumberOfJobs($id_company, $conn); ?></span></p>
        </div>
        <div class="container">
            <img src="../images/logoimg.jpg"> <p>Application for Jobs <br> <span></span></p>
        </div>
    </div>
...
<?php
// Function to get the number of jobs posted by the company
function getNumberOfJobs($company_id, $conn) {
    $jobsQuery = "SELECT COUNT(*) AS totalJobs FROM job_post WHERE id_company = '$company_id'";
    $jobsResult = $conn->query($jobsQuery);

    if ($jobsResult->num_rows > 0) {
        $jobsData = $jobsResult->fetch_assoc();
        return $jobsData['totalJobs'];
    } else {
        return "Error fetching job data: " . $conn->error;
    }
}
